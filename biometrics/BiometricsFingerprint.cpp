...
static const char *kHALModules[] = {HAL_MODULES};
static const size_t kHALModulesSize = sizeof(kHALModules) / sizeof(char *);
...
BiometricsFingerprint::BiometricsFingerprint() : mClientCallback(nullptr), mDevice(nullptr) {
    int i;
    const char *hw_mdl_id;
    sInstance = this; // keep track of the most recent instance
    for (i=0; i < kHALModulesSize; i++) {
        hw_mdl_id = kHALModules[i];
        mDevice = openHal(hw_mdl_id);
        if (!mDevice) {
            ALOGE("Can't open HAL module, module ID %s", hw_mdl_id);
        } else {
            ALOGI("Opened fingerprint HAL, module ID %s", hw_mdl_id);
            break;
        }
    }
}
